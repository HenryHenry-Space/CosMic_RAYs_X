...
class EElab_test_gri_2(SingleArmEnv):
    """
    ...
    def staged_rewards(self):
        ...
        # r_reach = 0
        r_reach = 0.5*(1 - np.tanh(10.0 * dist)) # + np.tanh(10.0 * self.old_dis))
        touch_sensor = self.sim.data.get_sensor('touchsensor')


        if touch_sensor != 0:
            touch_punish = np.clip(touch_sensor,0.1,0.5)
            r_reach -=touch_punish
            # r_reach -=0.1
